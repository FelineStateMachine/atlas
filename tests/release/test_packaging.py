...
class NativePackagingTest(unittest.TestCase):
    ...
    def test_linux_package_registers_atlas_mime(self) -> None:
        ...
        magic = mime_type.find("mime:magic", namespace)
        self.assertEqual(magic.attrib["priority"], "90")
        signature = magic.find("mime:match", namespace)
        self.assertEqual(signature.attrib["offset"], "0")
        self.assertEqual(signature.attrib["value"], "PK")
        bootstrap = signature.find("mime:match", namespace)
        self.assertEqual(bootstrap.attrib["offset"], "30")
        self.assertEqual(bootstrap.attrib["value"], "atlas.json")
