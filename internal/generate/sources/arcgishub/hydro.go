...
// The membership join. A capture that carries the national subwatersheds can
// say, of each zone the city itself curated, which subwatershed its ground lies
// in -- computed here from the captured polygons, so re-curating the sentence
// never needs a re-fetch and the capture stays the one unit of versioning.
//
// The claim is made only when all of a zone's sampled ground agrees on one
// answer. A zone that straddles subwatersheds says nothing rather than
// something misleading: one earned sentence beats a wall of uncorrelated prose.

// hydroUnit is one subwatershed as captured: its code, its name, and its ground
// in true degrees.
...
// hydroIndex answers which subwatershed a position lies in.
...
// makes any claim.
func buildHydroIndex(raw *capture) *hydroIndex {
	for _, captured := range raw.Datasets {
		if captured.Slug != slugSubwatersheds {
			continue
		}
		index := &hydroIndex{}
		for _, row := range captured.Features {
			code, name := row.Fields["huc12"], row.Fields["name"]
			if code == "" || name == "" || row.Geometry.Type != geometryRings {
				continue
			}
			index.units = append(index.units, hydroUnit{code: code, name: name, rings: row.Geometry.Rings})
		}
		return index
	}
	return nil
...
// locate answers the subwatershed holding a position, if any does. Units arrive
// in capture order, so a position on a shared boundary answers deterministically
// rather than by whichever unit was tried first.
...
// pointInRings is even-odd ray casting over MultiPolygon nesting: crossing an
// outer ring enters ground, crossing a hole leaves it again.
...
// featureAnchor is one row's representative position in true degrees: the
// centroid of its largest outer ring, or the middle vertex of its longest line.
// A point row is its own anchor, though no point dataset zones.
...
// hydroClaims tracks, per zone, the subwatersheds its rows' ground lands in,
// and whether any of it landed nowhere.
...
// claimSamples is how many boundary positions a row volunteers beside its
// anchor. The anchor alone would let a zone made of one citywide polygon claim
// whichever subwatershed its centroid happens to sit in; sampling the boundary
// catches the spread, and the cost stays flat no matter how detailed the ring.
const claimSamples = 16

// claimPositions is the ground a row answers for: its anchor, and an even
// sample of its outer-ring or line vertices.
...
// apply writes the membership onto a zone when every row agreed on one
// subwatershed: a short sentence for the card, and the code as the
// machine-readable key beside it.
func (c *hydroClaims) apply(index *hydroIndex, zone *doc.Feature) {
