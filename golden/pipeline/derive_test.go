...
// planned is one lens's plan, and the pyramid the reference left for it.
type planned struct {
	plan      tiles.Plan
	...
	for _, ref := range store.Volumes() {
		...
		for _, world := range document.Worlds {
			...
			for _, lens := range world.Lenses {
				...
				out = append(out, planned{plan: plan, reference: reference})
			}
		}
		if len(out) == 0 {
			t.Fatalf("%s planned no pyramid", volume)
		}
		return out
	}
