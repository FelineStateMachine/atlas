...
(`.github/workflows/golden.yml`, on main and the clean-room branches, on
**ubuntu, macOS and Windows**). CI spells it `go run ./golden/harness`, which
is the same run: the Windows runner has no make. A run prints one line per
gate, then the waiver file, then a count:
...
working tree is LF everywhere and the captured fixtures are marked as data, so
no checkout can rewrite a byte between the capture and the comparison. The CI
...
| --- | --- | --- |
| `semconv-codegen` | M7 | The conventions have one machine-readable source, `spec/registry.yaml`, and three derivations: `format/semconv/registry_gen.go`, `analysis/semconv/keys.ts`, and `docs/semconv/REGISTRY.md` (§8). The gate is that the derivations are current — committed bytes equal generated bytes — and that generation is byte-stable across runs. It stands where the registry's prose-twin agreement test used to. Runs today. |
