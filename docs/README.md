...
| `app.md` — route table, session schema, region/partial map, the state-placement rule, the hostenv contract and the three-host story | [`app.md`](app.md) | Complete for what is built. §11 names the one host that is not: the Wails desktop shell of §3.4 is a later wave, and the route table's `/static` mount answers `404` until the seam is mounted. |
...
3. **The next wave's documents.** `app.md` §11 and `render-seam.md` §10 are
   ...
   it owns that edit.
4. **The old tree's prose.** §8's comment policy says the reference
   implementation's comment trail stays readable on the `golden-reference` tag
   and that no new file references it as documentation. Worth a grep at
   archival time.
